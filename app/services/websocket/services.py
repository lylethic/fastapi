...
async def check_user_statuses(
    ...
    while True:
        ...
        if not chats:
            return
        user_chat_guids = set(chats.keys()) & {
            str(chat.guid) for chat in current_user.chat
        }
        ...
        await asyncio.sleep(
            settings.SECONDS_TO_SEND_USER_STATUS
...
async def mark_user_as_online(
    cache: aioredis.Redis,
    current_user: User,
    socket_manager: WebSocketManager = None,
    chat_guid: str = None,
):
    # set new redis key
...
async def get_message_by_guid(
    db_session: AsyncSession, *, message_guid: UUID
) -> Message | None:
    query = select(Message).where(
        and_(Message.guid == str(message_guid), Message.deleted.is_(False))
    )
...
async def mark_last_read_message(
    db_session: AsyncSession, *, user_id: str, chat_id: str, last_read_message_id: str
) -> ReadStatus | None:
    query = select(ReadStatus).where(
        and_(ReadStatus.user_id == user_id, ReadStatus.chat_id == chat_id)
    )
    result = await db_session.execute(query)
...
async def get_read_status(
    db_session: AsyncSession, *, user_id: str, chat_id: str
) -> ReadStatus | None:
    query = select(ReadStatus).where(
        and_(ReadStatus.user_id == user_id, ReadStatus.chat_id == chat_id)
    )
...
async def get_chat_id_by_guid(
    db_session: AsyncSession, *, chat_guid: UUID
) -> str | None:
    query = select(Chat.id).where(Chat.guid == str(chat_guid))
    result = await db_session.execute(query)
    chat_id: str | None = result.scalar_one_or_none()

    return chat_id


async def send_new_chat_created_ws_message(
    socket_manager: WebSocketManager, current_user: User, chat: Chat
):
    """
    ...
    # get friend guid
    friend_guid: UUID | None = next(
        (user.guid for user in chat.user if not user == current_user), None
    )

    if friend_guid is None:
        logger.error(
            "Friend guid not found",
            extra={"type": "new_chat_created", "friend_guid": friend_guid},
        )
        return
    ...
    target_websockets: set[WebSocket] = socket_manager.user_guid_to_websocket.get(
        str(friend_guid)
    )
    ...
    if target_websockets:
        ...
        await asyncio.gather(
            *[
                socket.send_json(jsonable_encoder(payload))
