...
async def new_message_handler(
    ...
    **kwargs,
    ...
    try:
        ...
        # Update chat timestamp after new message is created.
        ...
        await db_session.refresh(chat, attribute_names=["user"])  # ?

    except Exception as exc_info:
        await db_session.rollback()
        logger.exception(
            f"[new_message] Exception, rolling back session, detail: {exc_info}"
        )
        raise exc_info
    ...
    if notify_friend_about_new_chat:
        ...
        await send_new_chat_created_ws_message(
            socket_manager=socket_manager, current_user=current_user, chat=chat
        )
...
async def message_read_handler(
    ...
    **kwargs,
    ...
    message: Message | None = await get_message_by_guid(
        db_session, message_guid=message_guid
    )
    if not message:
        await socket_manager.send_error(
            f"[read_status] Message with provided guid [{message_guid}] does not exist",
            websocket,
        )
    chat_guid = str(message_read_schema.chat_guid)
    if chat_guid not in chats:
        await socket_manager.send_error(
            f"[read_status] Chat with provided guid [{chat_guid}] does not exist",
            websocket,
        )
        return
    chat_id = chats.get(chat_guid)
    ...
    if read_status:
        outgoing_message = {
            ...
            "last_read_message_created_at": str(message.created),
        }
...
async def user_typing_handler(
    ...
    **kwargs,
    ...
    if chat_guid not in chats:
        await socket_manager.send_error(
            f"[user_typing] Chat with provided guid [{chat_guid}] does not exist",
            websocket,
        )
        return
...
async def chat_deleted_handler(
    ...
    **kwargs,
    ...
    if chat_guid not in chats:
        await socket_manager.send_error(
            f"[chat_deleted] Chat with provided guid [{chat_guid}] does not exist",
            websocket,
        )
        return
